...
def bgr_to_hsv_numpy_hue_shift(image, saturation_factor=2.0):
    ...
    #
    # color_max = np.maximum(np.maximum(r, g), b)
    # color_min = np.minimum(np.maximum(r, g), b)
    ...
    h_final = np.round(h / 2)
    s_final = np.round(s * 255)
    v_final = np.round(v * 255)

    # updated hue value

    h_final = (h_final + 18) % 180

    # np.clip  - to keep the value under 255
    s_final = np.clip(s_final * saturation_factor, 0, 255)
    ...
    print("Method 4 (Basic NumPy with Hue shift) Time: {:.3f}s\n\n".format(final_time))
